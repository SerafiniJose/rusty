...
impl Drop for Stream {
    /// Stop and close the oboe stream BEFORE its backing C++ `AudioStream` and the
    /// boxed data callback are freed.
    ///
    /// oboe 0.4.6's `AudioStreamAsync` has no `Drop` impl, so dropping the stream
    /// deletes the C++ `oboe::AudioStream` (via `AudioStreamHandle`) without first
    /// stopping it. The AAudio data callback runs on a separate ("AudioTrack")
    /// thread and can fire after the callback context has been freed, dereferencing
    /// freed memory inside `oboe::AudioStream::fireDataCallback` (observed SIGSEGV,
    /// null-ptr deref, on every session teardown). `stop()` blocks until the
    /// in-flight callback quiesces and `close()` releases the AAudio stream while
    /// the callback context is still valid, eliminating the use-after-free.
    fn drop(&mut self) {
        match self {
