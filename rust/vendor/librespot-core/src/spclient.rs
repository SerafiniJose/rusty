...
impl SpClient {
    ...
    pub async fn client_token(&self) -> Result<String, Error> {
        ...
        // The client token has to be minted for the same client id and platform the
        // session goes on to present, because login5 turns down a request whose
        // token was issued to a different client — it answers BAD_REQUEST before it
        // ever looks at the credential. Deriving the id from the host OS instead,
        // as this used to, silently pinned the token to Android on an Android
        // build no matter which id the session was configured with.
        //
        // A Connect receiver has to present a desktop client id, since that is the
        // only one that can redeem a credential handed over by a desktop
        // controller, so a mobile host announces the desktop platform too. This is
        // the identity go-librespot presents on every platform it runs on.
        let os = match OS {
            "macos" => "macos",
            "windows" => "windows",
            _ => "linux",
        };
        client_data.client_id = self.session().client_id();
