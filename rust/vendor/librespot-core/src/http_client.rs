...
impl HttpClient {
    pub fn new(proxy_url: Option<&Url>) -> Self {
        ...
        // The platform announced here has to agree with the client id the session
        // presents, because Spotify validates the pair: a desktop/web client id
        // arriving under an "Android" user agent is turned down by login5 as a
        // BAD_REQUEST before the credential is even looked at. A Connect receiver
        // has to present a desktop client id — that is the only one that can
        // redeem a credential handed over by a desktop controller — so it
        // announces the matching platform too, which is also what go-librespot
        // does on every platform it runs on.
