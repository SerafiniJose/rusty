...
// A Connect receiver presents a desktop client id — the only one that can redeem
// a credential handed over by a desktop controller — and announces the desktop
// platform to match. The version it claims has to belong to that same client, or
// the identity is mixed again: a mobile version number under a desktop client id
// is a combination no real client sends, and mismatched identity is exactly what
// Spotify turns down. So the desktop version is reported on every host.
pub fn spotify_version() -> String {
    SPOTIFY_VERSION.to_string()
}

pub fn spotify_semantic_version() -> String {
    SPOTIFY_SEMANTIC_VERSION.to_string()
}
